// commande a tapé : gcc selection.c -o selection; .\selection.exe
...
// Fermeture du fichier valeurs.txt
    ...
    Pour i allant de 0 à nbr donnée présente dans le fichier
        - indice_p >- j
        - Pour j allant de i+1 à nbr donnée présente dans le fichier
            Si  T[indice_p] > T[j] alors
                indice_p <- j
        - Échanger T[i] et T[indice_p]
    */
    for(int i = 0; i < nbr_val_present_fichier_txt; i++)
    {
    ...
    FILE* fichier_val_trier = NULL;// Effectuer pour tester
